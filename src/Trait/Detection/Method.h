...
#define SCL_META_METHOD_DETECTION( method, Shortname ) \
    ...
    using Shortname ## MethodUnstrictOperation = decltype( ::std::declval< _Type >(). method ( ::std::declval< _Arguments >() ... ) ); \
     \
    template < typename _Type, typename ... _Arguments > \
    using Shortname ## MethodStrictOperation = decltype( (::std::declval< _Type >() .* ::std::integral_constant< ::ScL::MemberSignature< _Type, Shortname ## MethodUnstrictOperation< _Type, _Arguments ... >( _Arguments ... ) >, (&::std::decay_t< _Type >::method) >::value)( ::std::declval< _Arguments >() ... ) ); \
     \
    template < typename _Type, typename ... _Arguments > \
