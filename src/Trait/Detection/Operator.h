...
#define SCL_META_PREFIX_UNARY_OPERATOR_DETECTION(symbol, Invokable)                 \
    ...
    using Invokable##UnstrictOperation = decltype(symbol ::std::declval<_Right>()); \
                                                                                    \
    template <typename _Right>                                                      \
    using Invokable##MemberStrictOperation = decltype((::std::declval<_Right>()     \
        .*::std::integral_constant<                                                 \
            ::ScL::MemberSignature<_Right, Invokable##UnstrictOperation<_Right>()>, \
            (::ScL::MemberSignature<_Right,                                         \
                Invokable##UnstrictOperation<                                       \
                    _Right>()>)(&::std::decay_t<_Right>::operator symbol)>::value)());
...
#define SCL_META_POSTFIX_UNARY_OPERATOR_DETECTION(symbol, Invokable)                 \
    ...
    using Invokable##UnstrictOperation = decltype(::std::declval<_Left>() symbol);   \
                                                                                     \
    template <typename _Left>                                                        \
    using Invokable##MemberStrictOperation = decltype((::std::declval<_Left>()       \
        .*::std::integral_constant<                                                  \
            ::ScL::MemberSignature<_Left, Invokable##UnstrictOperation<_Left>(int)>, \
            (::ScL::MemberSignature<_Left,                                           \
                Invokable##UnstrictOperation<_Left>(                                 \
                    int)>)(&::std::decay_t<_Left>::operator symbol)>::value)(        \
        ::std::declval<int>()));

#define SCL_META_BINARY_OPERATOR_DETECTION(symbol, Invokable)                                   \
    template <typename _Left, typename _Right>                                                  \
    using Invokable##UnstrictOperation = decltype(::std::declval<_Left>()                       \
            symbol ::std::declval<_Right>());                                                   \
                                                                                                \
    template <typename _Left, typename _Right>                                                  \
    using Invokable##MemberStrictOperation = decltype((::std::declval<_Left>()                  \
        .*::std::integral_constant<                                                             \
            ::ScL::MemberSignature<_Left, Invokable##UnstrictOperation<_Left, _Right>(_Right)>, \
            (::ScL::MemberSignature<_Left,                                                      \
                Invokable##UnstrictOperation<_Left, _Right>(                                    \
                    _Right)>)(&::std::decay_t<_Left>::operator symbol)>::value)(                \
        ::std::declval<_Right>()));

#define SCL_META_GLOBAL_BINARY_OPERATOR_DETECTION(symbol, Invokable)                             \
    template <typename _Left, typename _Right>                                                   \
    using Invokable##GlobalStrictOperation = decltype((                                          \
        ::std::integral_constant<Invokable##UnstrictOperation<_Left, _Right> (*)(_Left, _Right), \
            (Invokable##UnstrictOperation<_Left, _Right> (*)(_Left, _Right))(                    \
                &operator symbol)>::value)(::std::declval<_Left>(), ::std::declval<_Right>()));

#define SCL_META_POSTFIX_OPERATOR_DETECTION_WITH_ARGUMENT(symbol, Invokable)               \
    template <typename _Type, typename _Argument>                                          \
    using Invokable##UnstrictOperation = decltype(::std::declval<_Type>().operator symbol( \
        ::std::declval<_Argument>()));                                                     \
                                                                                           \
    template <typename _Type, typename _Argument>                                          \
    using Invokable##MemberStrictOperation = decltype((::std::declval<_Type>()             \
        .*::std::integral_constant<                                                        \
            ::ScL::MemberSignature<_Type,                                                  \
                Invokable##UnstrictOperation<_Type, _Argument>(_Argument)>,                \
            (::ScL::MemberSignature<_Type,                                                 \
                Invokable##UnstrictOperation<_Type, _Argument>(                            \
                    _Argument)>)(&::std::decay_t<_Type>::operator symbol)>::value)(        \
        ::std::declval<_Argument>()));

#define SCL_META_POSTFIX_OPERATOR_DETECTION_WITH_ARGUMENTS(symbol, Invokable)              \
    template <typename _Type, typename... _Arguments>                                      \
    using Invokable##UnstrictOperation = decltype(::std::declval<_Type>().operator symbol( \
        ::std::declval<_Arguments>()...));                                                 \
                                                                                           \
    template <typename _Type, typename... _Arguments>                                      \
    using Invokable##MemberStrictOperation = decltype((::std::declval<_Type>()             \
        .*::std::integral_constant<                                                        \
            ::ScL::MemberSignature<_Type,                                                  \
                Invokable##UnstrictOperation<_Type, _Arguments...>(_Arguments...)>,        \
            (::ScL::MemberSignature<_Type,                                                 \
                Invokable##UnstrictOperation<_Type, _Arguments...>(                        \
                    _Arguments...)>)(&::std::decay_t<_Type>::operator symbol)>::value)(    \
        ::std::declval<_Arguments>()...));
